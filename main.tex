...
\begin{definition}[Vocabulaire]	
	\leavevmode
	\begin{itemize}
		\item L'espace vectoriel $V$ est \underline{l'espace de la representation}.
		\item La dimension de $V$ est le \underline{degree} (ou la \underline{dimension}) de la representation.
		\item Lorsque $\rho$ est injectif, la representation est dite \underline{fidèle}. Le group G se représente alors de manière concrète comme un sous-groupe de $GL(V)$.
...
\begin{examplebox}
	\begin{enumerate}
		\item La represantation \underline{triviale} (on representation unité).
		\begin{align*}
			\rho:\ G &\rightarrow GL(\CC)\simeq\CC^\ast\\
			g &\mapsto (\rho_g: id:\CC\rightarrow \CC\ x\mapsto x)
		\end{align*}
		\item Les représentation de degre 1: ce sont les \underline{homomorphisms} $\rho:G\rightarrow \CC^\ast$ puisque si $\dim V= 1$ alors $GL(V)\simeq\CC^\ast$.
		En effet les endomorphisms de $V$ sont les \underline{homothétis}: $f_\lambda: V\rightarrow V x\rightarrow \lambda x(\lambda\in \CC^\ast)$. Et $GL(V)\rightarrow  G^\ast$ est un isomorphisme $f_\lambda\rightarrow \lambda$.
