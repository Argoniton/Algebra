...
\section{Premieres definitions, représentations, yep isomorphisms et rep. iuductis)} % (fold)
...
\begin{definition}[Vocabulaire]	
	...
	\begin{itemize}
		...
		\item La dimension de $V$ est le \textdemp{degree} (ou la \textdemp{dimension}) de la représentation.
		\item Lorsque $\rho$ est injectif, la représentation est dite \textdemp{fidèle}. Le groupeG se représente alors de manière concrète comme un sous-groupe de $GL(V)$.
...
Soit $G$ un groupefini. On considère le $\CC$-espace vectoriel $\F (G)$ des fonctions complexes définies sur $G$ ($f:G\mapsto  \CC$) que l'ou munit de la structure hermitien donnée par le produit scalaire.
Pour $\varphi,\psi\in\F(G)$:

$$\expval{\varphi,\psi}=\frac 1{|G|}\sum_{g\in G}\overline{\varphi(g)}\psi(g).$$

On a: $\dim_\CC\F(G)=|G|$.
En effet, si $f\in\F(G)$ alors $f=\sum\limits_{g\in G}\lambda \Ind_g$ où \scalebox{0.8}{$\begin{array}{rcl}\Ind_g:G&\mapsto& \CC\\x&\mapsto& \scalebox{1}{\mbox{$\left\{\begin{array}{cl}1&\mbox{$x=g$}\\0& \mbox{sinon}\end{array}\right.$}}\end{array}$}
