...
\begin{definition}
	...
	telle que:
	\begin{enumerate}
		\item $\forall x\in V,\ e\cdot x=x$ où $x$ est l'element heutu de $G$
...
Ces deux définitions sont bien equivalents. En effet, si G opère sur $V$ de la manière linéaire a lois considérons l'application:
\begin{align*}	
	\rho:G &\rightarrow \ ? \\
...
Réciproquement, si $\begin{array}{rcl}\rho: G &\rightarrow & GL(V),\\ g &\mapsto &\rho_g\end{array}$ est un morphisme de groups, alors considérons: 
\begin{align*}
...
\begin{examplebox}
	\begin{enumerate}
		\item La represantation \textdemp{triviale} (on representation unité).
		\begin{align*}
			\rho:\ G &\rightarrow GL(\CC)\simeq\CC^\ast\\
			g &\mapsto (\rho_g: id:\CC\rightarrow \CC\ x\mapsto x)
		\end{align*}
		\item Les représentation de degre 1: ce sont les \textdemp{homomorphisms} $\rho:G\rightarrow \CC^\ast$ puisque si $\dim V= 1$ alors $GL(V)\simeq\CC^\ast$.
		...
		(Car si $g^n=e$ alors $\rho_{g^n}=(\rho_g)^n$). En particulier, ce sont des nombers complexs de mondle 1. $|\rho_g|=1$.
		\item Soient $\gS_m$ considere group simetrique et $(e_1,..., e_n)$ la base canonique de $\CC^n$. On definit une representation de degré $n$ de $\gS_n$ en posant:
		\begin{align*}
			...
			\right)
		\end{align*}
		\item La representation de permutation c'est un généralisation l'exemple précident. Soit $G\times X\rightarrow X$ une action $(g,x)\mapsto g\cdot x$ d'un group G sur un ensembele fini x. Soit V un C esp vect de dimension egale au cardinal de X, dune base indexée par les elements de X: $\{\eps_x,\ x\in X\}$. On a donc: $V=\bigoplus_{x\in x}\expval{\eps_x}=\bigoplus_{x\in X}\C_{\eps_x}$. On définit une re linéaire
...
En teims de matrices, ce la signifie que les matrices associés à la premiér represantion sont semblables à lems homologus dans la recoude, via la même matrice de ppage:
$$\forall g\in G, Mat(\rho'_g)=Mat(f)\times Mat(\rho_g)\times Mat(f)\dmo$$

($A,B$ Sembables si $\exists P:\ B=PAP\dmo$)

Si $\rho:G\rightarrow GL(v)$ est une represantation d'un group $G$ et si $W$ est un sous-espase vectoriel de V \textsc{Stable} par les differents automprphisms $\rho_g$ pour $g\in G$ i.e $\rho_g(W)\subset W$ (i.e $\forall g\in G, \forall w\in W, \rho_g(w)\in W$)
...
\begin{definition}
	Une représentation $\rho: G\rightarrow  GL(V)$ est dite \textsc{Irreductible} si les seuls sous-espaces stables de V sont $\{0\} $et$ V$.
\end{definition}

Ainsi les reparametrisation de \emph{degré 1} constituent des representations irréductibles particuliers.

\begin{theorem}[Théorème de Maschke]
	On géfinit le somme directe de representation de group fini $G$. Soient $h\rho:g\rightarrow GL(V)$ et $\rho;"G_j>GL(V')$ deux representations de $G$. On definit la somme directe
	$$\rho\bigoplus \rho'$$ comme etant:
	La rep. d'espace vectkoriel $V\bigoplus V'$ definit par:
	$\rho\bigoplus\rho': G+> GL(V\bigoplus V')\ g\mapsto(\rho\bigoplus')_g$ definit par $\forall n\in V, \forall v'\in V', (\rho\bigoplus \rho')_g(v+v')=\rho_g(v)=\rho'_g(v')$
	
	Toute représentation linéaire complexe de degre fini d'un group fini est somme directe de repr. inductives.
\end{theorem}

\begin{lemme}
	Tout sous-espace stable d'une rep. lin. complexe de degré fini d'un groupe fini admet un sous-espacce \emph{Supplémentaire Stable}.
\end{lemme}
\begin{proof}[Lemma]
	Il existe un produit dcalaire hermitien sur l'espace de la rep. stable sous l'action du groupe. En effect, si $\expval{\cdot,\cdot}$ désigne un produit scalaire que lonque sur V, le produit scalaire survant est stable par $\rho$:
	Pour $x,y\in V$:
	$$\expval{x,y}_\rho=\frac 1{|G|}\sum_{g\in G\expval\rho_g(x),\rho_g(y)}$$
	En effet, si $h\in G$, on a;
	$$\expval{\rho_h(x),\rho_h(y)}_\rho=\frac 1{|G|}\sum_{g\in G}\expval(\rho_h(x),\rho_g(\rho_h(g)))=\frac 1{|G|}\sum_{g\in G}\expval{\rho_{gh}(x),\rho_{gh}(y)} = \expval{x,y}_\rho$$ car $g\mapsto gh$ est une bijection de $G$ sur lui-méme.
	Si $W$ est un sous-espace vectorie de V stable sous l'action d  alors le supement orthogonal de W est lui aussi stable sous l'action puique $x$ orthogonal a $W$ $\Leftrightarrow$ $\rho_g(x)$ orthogonal $\rho_g(W)=W$
\end{proof}
\begin{proof}
	On fait une recuneuce sur la dimension de l'esp vect de la rep.
	Si dim V=1 on si V est li ok\\
	Si dim v \geq 2 et V est non ined alors V pose de un sous --representation W. Distincte de $\{0\}$ et V.
	Si $\expval{\cdot, \cdot}$ est un proudit scalaire ermitian sur V invariant sous l'action de $G$, le supplémentaire orthogonal $W\perp$ de W est lui aussi stable sous l'action de G.
	
	On a lors: $V=W\bigoplus W^\perp$ et W et $W\perp$ sont de dimensions < dim V.
	L'hypothese de recurence permet de les décomposer comme des sommes disuctes de rep in ce que prouve gu'on peut en faire antant de V.
\end{proof}
